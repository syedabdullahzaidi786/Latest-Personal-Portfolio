...
export async function GET() {
  ...
  try {
    const { rows } = await pool.query(
      'SELECT id, image, title, description, url, "createdAt" FROM "Blog" ORDER BY "createdAt" DESC'
    );
    return NextResponse.json({ blogs: rows });
  } catch (error: any) {
    console.error('[blogs GET]', error);
    return NextResponse.json({ blogs: [], error: error?.message }, { status: 200 });
  } finally {
    await pool.end();
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO "Blog" (image, title, description, url, "createdAt", "updatedAt")
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    if (rows.length === 0) return NextResponse.json({ error: 'Blog post not found' }, { status: 404 });
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const { rowCount } = await pool.query('DELETE FROM "Blog" WHERE id=$1', [Number(id)]);
    if (rowCount === 0) return NextResponse.json({ error: 'Blog post not found' }, { status: 404 });
