...
import { Trophy, ExternalLink, Star, Award, Medal, ArrowUpRight } from 'lucide-react';
...
function AchievementCard({ item, index }: { item: Achievement; index: number }) {
  ...
  return (
    ...
    >
      ...
      {/* image / placeholder — clickable → detail page */}
      <Link href={`/achievements/${item.id}`} className="block relative">
        <div className="relative h-64 overflow-hidden bg-[#0a0a12] flex-shrink-0">
          {item.image ? (
            <>
              <Image
                src={item.image}
      ...
      <div className="flex flex-col flex-1 p-5 gap-3">
        <Link href={`/achievements/${item.id}`} className="group/title">
          <h3 className="text-sm font-bold text-white leading-snug line-clamp-2 group-hover/title:text-blue-300 transition-colors duration-200">
            {item.title}
          </h3>
        </Link>
